...
image: https://bytebytego.com/social2.png
---
...
In many cases, such comparisons are made using two nested for-loops, which takes
O
(
n
2
...
O(n
2
) time, where n denotes the length of the data structure. In the code snippet below, i and j are two pointers used to compare every two elements of an array:
...
Two-pointer algorithms usually take only
O
(
n
)
O(n) time by eliminating the need for nested for-loops. There are three main strategies for using two pointers.
